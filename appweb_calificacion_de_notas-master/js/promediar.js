...
function agregarNotas(){
    ...
    if(nom===""){
     ...
     //Validadr el campo apellido
     if(ape===""){
        
        swal("Ingrese el apellido del Estudiante", "El campo de apellido no puede ser vacio", "warning");return false;
    }
    ...
    //Validar las notas, no mayor de 21 puntos, solo asta 25
    ...
    fila.innerHTML=`<td> ${nom} </td> <td> ${ape} </td><td> ${not1} </td><td> ${not2} </td><td> ${not3} </td><td> ${prom.toFixed(1)} </td><td> ${obs} </td>`;
